...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const artistsPromise = getAllArtists().then(artists => artists.map(artist => ({ url: `${baseUrl}/tatuador/${artist.slug}/tatuajes`, lastModified: new Date().toISOString() })))

  const tattoosPromise = filterAndPaginateTattoos({}, { size: 9999, page: 1 }).then(tattoos => tattoos.data.map(tattoo => ({ url: `${baseUrl}/tatuajes/${tattoo.slug}`, lastModified: new Date().toISOString() })))
