...
const formatErrors = <T extends { [key: string]: string }>(
  error: ZodError<T>
) => {
  ...
  for (const key in formatedErrors) {
    ...
    const issue =
      formatedErrors[key as keyof Omit<typeof formatedErrors, '_errors'>]
...
export const createPageAction = async (
  ...
  try {
    ...
    delayRevalidate()
    return {
      message: 'La página se creó correctamente',
      status: 1,
      page: newPage,
    }
  } catch (err) {
    console.error(err)
    return {
      message: 'Ocurrió un error al intentar crear la página',
      status: -1,
    }
  }
}
...
export const updatePageAction = async (
  ...
  try {
    ...
    console.error(err)
    return {
      message: 'Ocurrió un error al intentar editar la página',
      status: -1,
    }
  }
}
...
export const deletePageAction = async (
  ...
  if (!id)
    return {
      message:
        'Algo salió mal al obtener la página, porfavor inténtalo denuevo o contacta a soporte',
      status: -1,
      fieldIssues: {},
    }
  ...
  try {
    ...
    return {
      message:
        'Ocurrió un error al intentar borrar la página, porfavor inténtalo denuevo o contacta a soporte (h)',
      status: -1,
    }
  }
}
...
const generatePageSlug = async (title: string) => {
  ...
  while (isValid) {
    isValid = Boolean(
      await prisma.page.findFirst({ where: { slug: treatedTitle } })
    )
