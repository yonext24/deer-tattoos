import { HIDDEN_PRODUCT_TAG, SHOPIFY_GRAPHQL_API_ENDPOINT, TAGS } from '@lib/utils/consts';
import { isShopifyError } from '@lib/types/type-guards';
...
export async function shopifyFetch<T>({
  ...
  try {
    const cacheToUse: RequestCache | false = (() => {
      if (revalidate) return false
      if (cache) return cache
      if (revalidate && !cache) return 'reload'
      return 'default'
    })()
    const next = {
      tags,
...
export async function revalidate(req: NextRequest): Promise<NextResponse> {
  ...
  const digest = crypto
    .createHmac(
      'sha256',
      process.env.SHOPIFY_WEBHOOK_SIGNATURE as string
    )
    .update(data)
    .digest('base64');
